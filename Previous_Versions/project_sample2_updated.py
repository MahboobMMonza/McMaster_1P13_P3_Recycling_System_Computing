...
def bot_align(home=False):
    ...
    while readings[0] + readings[1] != 2:
        ...
        bot.stop()
        readings = bot.line_following_sensors()
    while home and not check_home(0.05):
...
def load_bottle(bottle_count, bottle_id):
    ...
    DROP_OFF = (0, -0.58 + (0.05 + 0.02 * (not(bottle_id == 2 or bottle_id == 4))) * bottle_count, 0.55)
    PICK_UP = (0.63, 0, 0.22)
...
def dispense_containers(bottles, has_bottle=False, bottle_info=None):
    ...
    while bottle_count < 3 and bottles:
        info = extract_container_info(bottles.pop())
        if info[0] + total_mass < 90 and info[1] == main_dest:
            load_bottle(bottle_count, main_dest)
            bottle_count += 1
        else:
            return True, main_dest, info
    return False, main_dest, info
